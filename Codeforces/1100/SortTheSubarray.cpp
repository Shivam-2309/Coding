...
void solve(){
    ...
    while(leftMost < n){
        ...
        if(leftMost == n-1) break;
        leftMost++;
    }
    while(rightMost >= 0){
        if(a[rightMost] != b[rightMost]){
            break;
        }
    ...
    while(leftMost >= 0){
        if(leftMost == 0 || b[leftMost-1] <= b[leftMost]){
            leftMost = max(0ll, leftMost-1);
            if(leftMost == 0) break;
        }
        else{
            break;
    ...
    while(rightMost < n){
        if(rightMost == n-1 || b[rightMost] <= b[rightMost + 1]){
            rightMost = min(n-1, rightMost+1);
    ...
    cout << leftMost+1 << " " << rightMost+1 << endl;
}
