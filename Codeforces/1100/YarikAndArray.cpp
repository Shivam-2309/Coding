...
// ll dp[NUM];
 // **************************************Helpful functions************************************
ll maxSubarraySum(ll l, ll r, vector<ll> &arr) {
    ll maxi = -INF; 
    ll sum = 0;
...
void solve(){
    ...
    ll maxSum = -INF;
    for(ll r=0; r<n;){
        if((r == n-1) || (abs(v[r]%2) == abs(v[r+1]%2))){
            // l -> r is a valid subarray to apply kadanes algorithm on
            // cout << r << " ";
            ll sum = maxSubarraySum(l, r, v);
            // cout << sum << " ";
