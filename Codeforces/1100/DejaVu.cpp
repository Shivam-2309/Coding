...
// ll dp[NUM];
 // **************************************Helpful functions************************************
void solve(){
    // Important thing to note is that, reoccurance of a number clearly does no makes sense, because, the number that would have been creater after
    // Adding 2^x-1 now would never be divisible by 2^(x-1)
    // Formally we can say, that if a power of two is added with a number that is one less power of two than
    // the previous number then I can say that the new number formed can never now be a dividible by 2^x
    ll n, q;
    cin >> n >> q;
