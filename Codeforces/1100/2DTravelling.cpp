...
void solve(){
    ...
    ll minDist1 = INF;
    for(ll i=0; i<k; i++){
        ll dist = abs(v[i].first - v[a].first) + abs(v[i].second - v[a].second);
        minDist1 = min(minDist1, dist);
    }

    ll minDist2 = INF;
    for(ll i=0; i<k; i++){
        ll dist = abs(v[i].first - v[b].first) + abs(v[i].second - v[b].second);
        minDist2 = min(minDist2, dist);
    }

    ll minDist3 = abs(v[a].first - v[b].first) + abs(v[a].second - v[b].second);

    ll ans = min(minDist1+minDist2, minDist3);
