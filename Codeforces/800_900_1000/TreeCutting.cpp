...
ll dfs(ll node, ll parent, vector<ll> adj[], ll &cuts, ll x){
    ll size = 1;

    for(auto it: adj[node]){
        if(it != parent){
            size += dfs(it, node, adj, cuts, x);
        }
    }
    ...
    if(size >= x){
        // Ek cut lga diya 
        cuts++;
        // Ab uss subtree ke corresponding nothing will come 
        return 0;
    }
    return size;
}

bool isValid(ll x, ll k, vector<ll> adj[]){
    ll cuts = 0;

    ll size_of_root_tree = dfs(1, -1, adj, cuts, x);
