...
void solve(){
    ll n;
    cin >> n;
    // Because we know that for two number a and b, thier LCM is smallest when (say b>=a), b%a == 0
    // we are given tha a+b == n, thus, we can make a conclusice statement
    // that we need the value of a such that (b+a)%a == 0 or simply n%a is 0
    // For this find all the divisors of n
    vll divisors;
    ...
    // We know the minimum answer is surely n-1
    // I need to chekc others, other possibilites are those which have n%a == 0 (all divisors of a)
    // Learning -> For any two numbers the minimum LCM is when higher number is mulitple of the smaller number(can be proved mathematically)
    ll a = 1;
    ll b = n-1;
    ll ans = n-1;

    for(auto &el: divisors){
        // If one is el, then second is n/el
        // and if one element is n then second element is ofcourse el*sec - el -> el*(sec - 1)
        ll sec = n/el;
        ll tempa = 1*el;
        ll tempb = el*(sec - 1);

        ll lcm = __lcm(tempa, tempb);

        // if lcm is smaller simply update our a and b
        if(lcm < ans){
            ans = lcm;
            a = tempa;
            b = tempb;
