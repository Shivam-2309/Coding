...
void solve(){
    ...
    ll total_sum = 0;
    for(ll i=0; i<n; i++){
        total_sum = (total_sum + v[i])%mod7;
    }
 
    ll max_sum = maxSubArray(v);
    ...
    if(max_sum <= 0){
        // cout << "CHECK" << endl;
        // cout << total_sum + mod7 << endl; 
        if(total_sum < 0){
            cout << (total_sum+mod7)%mod7<< endl;
        }
        else{
            cout << total_sum%mod7 << endl;
        }
    ...
    for(ll i=1; i<=k; i++){
        total_sum = (total_sum + max_sum + mod7)%mod7;
        max_sum = (max_sum + max_sum)%mod7;
    }   
